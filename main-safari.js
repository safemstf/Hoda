// Safari-compatible version using script tag instead of ES modules
// Global variables
let chat = null;
...
async function loadModel() {
    try {
        ...
        // Check if WebLLM is available globally
        if (typeof window.webllm === 'undefined') {
            throw new Error('WebLLM not loaded. Please check the script tag.');
        }
...
// Initialize when WebLLM is loaded
function initializeApp() {
    if (typeof webllm !== 'undefined') {
        loadModel();
    } else {
        updateStatus('Waiting for WebLLM to load...', 'loading');
